...
/*
 * Sets Colour Scheme on booking table
 */
...
{		         
			...
			boolean hasFocus, int row, int column)
	{
		...
		if (column == 0 || column == 1 ){	
			cell.setBackground( instColors[(row/2)%4]);  	//installer color
			cell.setForeground( DkBlue );		          	//font color
			cell.setFont(cell.getFont().deriveFont(Font.BOLD));	
			setHorizontalAlignment(SwingConstants.CENTER);
			return cell;
		}else if (column == 2 || column == 8 ){          	//weekend
